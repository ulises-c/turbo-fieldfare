...
| Maximum context | App: 4K, 8K, 16K, 32K, 64K, 96K, 128K, 192K, 256K | `--max-context` | CLI and app: 8K; server: 16K | Sets prompt plus response capacity, and 8K is what leaves room for an image and its prompt. The app shows the FP16 KV-memory delta, computed for the installed architecture. The server defaults higher still because agent clients routinely send prompts near 8K on their own. The server accepts 4K, 8K, 16K, 32K, 64K, 96K, 128K, 192K, or 256K — 256K is the native window of both supported families; the CLI accepts any positive integer and is bounded by memory rather than by a list. KV is allocated at the cap, not the prompt length, so this is the memory dial: see [Memory expectations](MEMORY_EXPECTATIONS.md). `--prefill off` is rejected when the unringed KV would exceed 16 GiB, which for Gemma 4 means above 64K; Qwen 3.6 has no sliding-window layers, so the ring is a no-op for it and every context stays available. |
